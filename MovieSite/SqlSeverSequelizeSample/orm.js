...
sampleDb.sync({ force: true }).then(function() {
  console.log('\nCreated database schema from model.');

  // Create demo: Create a User instance and save it to the database
  User.create({ firstName: 'Anna', lastName: 'Shrestinian' }).then(function(user) {
    ...
    Task.create({
      ...
      user.setTasks([task]).then(function() {
        console.log("\nAssigned task '" + task.title + "' to user " + user.firstName + ' ' + user.lastName);
              ...
              .then(function() {
                ...
                Task.destroy({
                  ...
                  Task.findAll().then(function(tasks) {
                    console.log('Tasks in database after delete:', JSON.stringify(tasks));
                    console.log('\nAll done!');
                  });
                });
              });
