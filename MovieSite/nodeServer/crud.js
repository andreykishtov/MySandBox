...
function Update(name, location, callback) {
  ...
  request = new Request('UPDATE TestSchema.Employees SET Location=@Location WHERE Name = @Name;', function(
    err,
    rowCount,
    rows
  ) {
...
function Delete(name, callback) {
  ...
  request = new Request('DELETE FROM TestSchema.Employees WHERE Name = @Name;', function(
    err,
    rowCount,
    rows
  ) {
...
function Read(callback) {
  ...
  request = new Request('SELECT Id, Name, Location FROM TestSchema.Employees;', function(
    err,
    rowCount,
    rows
  ) {
...
function Complete(callback, err, result) {
  if (err) {
    callback(err);
  } else {
    console.log('Done!');
...
connection.on('connect', function(err) {
  if (err) {
    ...
    async.waterfall([Start, Insert, Update, Delete, Read], Complete);
  }
});
