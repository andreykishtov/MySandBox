...
class App extends Component {
  ...
  render() {
    let icon = {left:'fa fa-chevron-left',right:'fa fa-chevron-right'}
    return (
      <Wrapper>
        <BookWrapper>
          <Icon style={icon.left} handleOnClick={() => this.ChangeBook('left')} />
          <Book
            {...this.state.currentBook}
            handleOnCheck={this.handleWishList}
            checked={this.state.checked[this.state.bookId]}
          />
          <Icon style={icon.right} handleOnClick={() => this.ChangeBook('right')} />
        </BookWrapper>
